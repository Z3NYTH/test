...
if __name__ == "__main__":
    ...
    floor.Texture(ImageTexture("./grass.jpg"))
    floor.Kd(0.8)
    floor.Ka(0.3)
    floor.Ks(0.2)
    floor.Reflexion(0.1)
    ...
    back.Texture(CheckerboardTexture(scale=10, even=Color(0.8, 0.8, 0.8), odd=Color(0.5, 0.5, 0.5)))
    back.Kd(0.7)
    back.Ka(0.3)
    back.Ks(0.1)
    scene.add_object(back)
    s1 = Sphere(Vec3(2.5, 4.0, 0.0), 2.0)
    s1.Texture(ImageTexture("./earth.webp"))
    s1.Ks(0.6)
    s1.Kd(0.8)
    s1.Ka(0.3)
    scene.add_object(s1)
    s2 = Sphere(Vec3(-3.5, 3.0, 0.5), 3.0)
    s2.Texture(PerlinNoiseTexture(scale=2))
    s2.Reflexion(0.2)
    s2.Transparence(0.4)
    scene.add_object(s2)

    c3 = Cube(
        Vec3(3.5 - 1.0, 4.0 - 1.0, 5.0 - 1.0),
        Vec3(3.5 + 1.0, 4.0 + 1.0, 5.0 + 1.0),
    )
    c3.Texture(SolidColorTexture(Color(1.0, 1.0, 1.0)))
